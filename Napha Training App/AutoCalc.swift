...
struct AutoCalcView: View {
    ...
    @State private var autoCalc: Float = 0.0 // Change type to Float
    @State var calcPicker: String = "Sit-Ups"  // Should be String, not Float
    @Environment(\.dismiss) private var dismiss
 // Change type to Float
    @State var sitReach: Float = 0.0
    let filterOptions: [String] = ["Sit-ups", "Pull-Up", "2.4KM", "Sit & Reach", "Incline Pull-Up", "Standing Broad Jump"]

    func calculateSitReachGrade(age: Int, sex: Bool, sitReach: Int) -> String {
        ...
        VStack {
            ...
            Slider(value: calcPicker == "Sit-Ups" ? $sitUps : $sitReach, in: 0...100, step: 1.0)
                .scaleEffect(0.6)
                       ...
                       let grade = calcPicker == "Sit-Ups"
                           ? calculateSitUpsGrade(age: info.Age, sex: info.Gender, sitUps: Int(sitUps))
                           : calculateSitReachGrade(age: info.Age, sex: info.Gender, sitReach: Int(sitReach))
